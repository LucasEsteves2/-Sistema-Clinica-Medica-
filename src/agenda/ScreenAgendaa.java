...
public class ScreenAgendaa extends JFrame {
	...
	AgendaModel agendaTable = new AgendaModel();

	public ScreenAgendaa(Connection con) {
		setTitle("Agenda Medica");

		this.con = con;
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(ScreenAgendaa.class.getResource("/Imagens/calendar_icon_134849 (3).png")));
		...
		imgPaciente.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/doctor (4).png")));
		...
		lblInicio.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/pagina-inicial (1).png")));
		...
		imgPaciente_1_1_1.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/cancelar.png")));
		...
		imgPaciente_1_1.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/health-check.png")));
		...
		imgPaciente_1.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/examination.png")));
		...
		btnExcluir.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/trash.png")));
		...
		txtPesquisar.setBounds(424, 36, 491, 34);
		...
		btnEditar.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/edit.png")));
		...
		btnNovo.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/plus-black-symbol.png")));
		...
		btnPesquisar.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/search.png")));
		...
		btnPesquisar.setBounds(925, 36, 55, 34);
		panel_1.add(btnPesquisar);

		JButton btnLimpar = new JButton("");
		btnLimpar.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/trash.png")));
		btnLimpar.setForeground(Color.WHITE);
		btnLimpar.setFont(new Font("Segoe UI", Font.PLAIN, 16));
		btnLimpar.setDoubleBuffered(true);
		btnLimpar.setBorder(null);
		btnLimpar.setBackground(Color.RED);
		btnLimpar.setBounds(925, 36, 55, 34);
		...
		banco.exibirConsultas(agendaTable);
		...
		lblMenu.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/2.jpg")));
		lblMenu.setBounds(-1, 11, 243, 714);
		panel.add(lblMenu);

		// Instanciando meu proprio model (jtable)

		jTabelaPaciente.setModel(agendaTable);
		...
		adicionarPaciente(btnNovo, con);

		editarPaciente(btnEditar, jTabelaPaciente);

		pesquisarPressedKey(btnPesquisar, banco, btnLimpar, null);

		JLabel lblBuscarData = new JLabel("Buscar");
		lblBuscarData.setVisible(false);

		lblBuscarData.setIcon(new ImageIcon(
				ScreenAgendaa.class.getResource("/Imagens/calendar_clock_schedule_icon-icons.com_51085.png")));
		lblBuscarData.setHorizontalAlignment(SwingConstants.CENTER);
		lblBuscarData.setFont(new Font("Tahoma", Font.BOLD, 11));
		lblBuscarData.setComponentOrientation(ComponentOrientation.LEFT_TO_RIGHT);
		lblBuscarData.setBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null));
		lblBuscarData.setBounds(843, -2, 74, 27);
		panel_1.add(lblBuscarData);

		calendario = new JDateChooser(null, "dd/MM/yyyy");
		calendario.setBounds(990, 44, 102, 20);

		panel_1.add(calendario);

		JLabel lblBackground = new JLabel("");
		lblBackground.setIcon(new ImageIcon(ScreenAgendaa.class.getResource("/Imagens/aaaaaaaaaaaaaaa.jpg")));
		lblBackground.setBounds(0, 0, 1139, 681);
		panel_1.add(lblBackground);

		calendario(lblBuscarData);
		pesquisarClick(btnPesquisar, txtPesquisar, agendaTable, banco);
		...
		txtPesquisar.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (txtPesquisar.getText().trim().length() <= 1) // vai procurar de 3 em 3 digitos
				{
					...
					banco.pesquisarConsulta(nome, agendaTable);
	...
	public void pesquisarClick(JButton btnPesquisar, JTextField txtPesquisar2, AgendaModel agendaTable2,
			QueryBd banco) {
		...
		btnPesquisar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (txtPesquisar.getText().trim().equals("") && data.equals("")) {
					...
					banco.pesquisarConsultaDatas(data, agendaTable, calendario);
	...
	public void editarPaciente(JTable jTabelaPaciente) {
		...
		if (jTabelaPaciente.getSelectedRow() != -1) {
			...
			String horario = jTabelaPaciente.getValueAt(linha, 1).toString();
			String paciente = jTabelaPaciente.getValueAt(linha, 2).toString();
			String medico = jTabelaPaciente.getValueAt(linha, 3).toString();
			String atendimento = jTabelaPaciente.getValueAt(linha, 4).toString();
			String data = jTabelaPaciente.getValueAt(linha, 5).toString();

			int id2 = Integer.parseInt(id);

			Agenda lista = new Agenda(paciente, medico, data, horario, id2, atendimento);

			EditarAgenda editar = new EditarAgenda(con, agendaTable, lista);

			editar.setLocationRelativeTo(null);
			editar.setVisible(true);

		} else {
			JOptionPane.showMessageDialog(null, "Por Favor, Selecione uma Consulta", "Consulta Invalida!",
	...
	public void adicionarPaciente(JButton btnNovo, Connection con2)

	{
		btnNovo.addActionListener(new ActionListener() // Escutador
		{
			...
			public void actionPerformed(ActionEvent e) {
				...
				NovaAgenda novaAgenda = new NovaAgenda(con2, agendaTable);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (jTabelaPaciente.getSelectedRow() != -1) {
					...
					if (i == JOptionPane.YES_OPTION) {
						...
						String id = jTabelaPaciente.getValueAt(jTabelaPaciente.getSelectedRow(), 0).toString();

						agendaTable.removeLinha(jTabelaPaciente.getSelectedRow(), id, con, jTabelaPaciente); // metodo
																												// que
																												// exclui
																												// a
																												// linha
